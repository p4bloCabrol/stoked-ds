...
This means the core bundle (28 components) is **never affected** by integration code.
...
All animations use **CSS keyframes and transitions** with zero JavaScript runtime cost:

- **Skeleton** pulse and wave, **Spinner** rotation, **Progress** bar stripes — `@keyframes` in component CSS modules
- **Modal** and **Toast** enter/exit — CSS keyframes with `data-state` attributes, managed by a lightweight `useExitAnimation` hook
- **Tabs** indicator — CSS transitions on `left` and `width` properties
- **Card** hover/active — CSS `:hover` and `:active` pseudo-classes with `transform`
- **Button** loading spinner — CSS `@keyframes spin`

All motion respects `prefers-reduced-motion` via CSS `@media` queries.

### forwardRef Everywhere

All 28 components use `React.forwardRef`. This is non-negotiable for a design system:
...
pnpm lint               → ESLint with React hooks rules
pnpm typecheck          → TypeScript strict mode
pnpm test               → 408 unit tests (Vitest)
pnpm test:e2e           → 352 E2E tests (Playwright, 3 browsers)
pnpm build              → Production build
pnpm pack --dry-run     → Validate package contents
```
...
- **1 production dependency**: `clsx` (class utility)
...
| Unit tests | 359 passing (Vitest) |
| E2E tests | 352 passing (Playwright × 3 browsers) |
| Test-to-component ratio | 1:1 (every component has unit + E2E tests) |
| Production dependencies | 1 (clsx) |
